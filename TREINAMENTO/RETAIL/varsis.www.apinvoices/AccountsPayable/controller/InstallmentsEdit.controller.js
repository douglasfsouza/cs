sap.ui.define([
	...
	return EditPageBase.extend(`${THIS_DOMAIN}.controller.InstallmentsEdit`, {
		_datasource: null,
		_modified: false,
		_grid: null,
		...
		configurationFile: function() {
			return `${THIS_DOMAIN}/view/InstallmentsEdit.view.json`;
		},
		...
		onInit: function () {
            ...
            this._datasource = new Datasource("APInvoice");
		...
		_loadWithHolding: function(recordKey) {
			...
			var ds = new Datasource("APInvoiceTax");
		...
		_loadSettlement: function(recordKey) {
            ...
            this._updateModel({"settlementLoaded": false});
			...
			var ds = new Datasource("FinancialSettlement");
			...
			ds.query(criterias).then(
				(response) => {
					if(response.success)
					{
						...
						this._updateModel({"settlementLoaded": true});
					}
					else
					{
